...
void solve(){
    input(int,n);
    vpi pos_odd,neg_odd,even;
    forn(i,n){
        input(int,m);
        if((m&1)){
            if(m>0)pos_odd.pb({i,m});
            else neg_odd.pb({i,m});
        }
        else even.pb({i,m/2});
    }
    ...
    forn(i,len){
        if(!(i&1)){
            pos_odd[i].second=pos_odd[i].second/2;
        }
        else{
            if(pos_odd[i].second>=0)pos_odd[i].second=(pos_odd[i].second/2)+1;
            else pos_odd[i].second=(pos_odd[i].second/2)-1;
        }
        even.pb(pos_odd[i]);
    }
     len=sz(neg_odd);
    forn(i,len){
        if(!(i&1)){
            neg_odd[i].second=neg_odd[i].second/2;
        }
        else{
            if(neg_odd[i].second>=0)neg_odd[i].second=(neg_odd[i].second/2)+1;
            else neg_odd[i].second=(neg_odd[i].second/2)-1;
        }
        even.pb(neg_odd[i]);
    }
    sort(all(even));
