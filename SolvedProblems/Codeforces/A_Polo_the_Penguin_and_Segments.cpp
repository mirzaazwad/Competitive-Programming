...
void solve(){
    ...
    print((k-sum%k)%k);// the number of moves needed to make it divisible by k, 
    //if it is already divisible k-sum%k would give k and doing mod k on that would give 0
    //if not k-sum%k would give a value other than k or the number of moves required and doing mod k would give that value as
    //that value is less than k
