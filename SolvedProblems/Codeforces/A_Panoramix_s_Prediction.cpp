...
void sieve(){
    ...
    for(int i=3;i<=52;i+=2)isPrime[i]=true;//mark 3 and all odd numbers from 3 onwards as prime excluding the even numbers in the process
    for(int i=3;i*i<=52;i++){
        if(isPrime[i]){
            for(int j=i*i;j<=52;j+=2*i)isPrime[j]=false;//this marks all odd multiples as not prime, since the even multiples are already marked as not prime, its not needed
...
void solve(){
    ...
    if(isPrime[n] && isPrime[m]){
        ...
        forn(i,n){
            if(primeList[i]==n && i+1<len && primeList[i+1]==m){
                YES;
                return;
            }
        }
