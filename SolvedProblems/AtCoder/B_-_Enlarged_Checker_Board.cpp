...
void floodfill(int x, int y,char character)
{
    visited[x][y] = 1;
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        if(xNew < 0 || xNew >= N || yNew < 0 || yNew >= N  || visited[xNew][yNew])continue;
        skeleton[xNew][yNew] = character;
        if(character =='.')floodfill(xNew, yNew, '#');
        else if(character=='#')floodfill(xNew, yNew, '.');
...
void solve(){
    ...
    skeleton[0][0]='.';
    floodfill(0,0,'#');
    char checkerBoad[N*A+1][N*B+1];
    for(int i=0;i<N*A;i++)for(int j=0;j<N*B;j++)checkerBoad[i][j]='$';
    for(int i=0;i<N*A;i+=A){
        for(int j=0;j<N*B;j+=B){
            for(int a=0;a<A;a++)for(int b=0;b<B;b++)checkerBoad[i+a][j+b]=skeleton[i/A][j/B];
        }
    }
    for(int i=0;i<N*A;i++){
        for(int j=0;j<N*B;j++){
            cout<<checkerBoad[i][j];
        }
        down;
    }
}
